...
async function fetch_unprocessed_urls(level) {
    return new Promise((resolve, reject) => {
        const query = `select domain, url from crawl_status_2 where level=${level} and status=0 and domain='apple.com'`
...
async function get_root_domain(){
    return new Promise((resolve, reject) => {
        ...
        const limit = argv[2]
        const offset = argv[3]
        const query = `select name from domains left outer join crawl_status_2 on domains.name = crawl_status_2.domain where domain is NULL LIMIT ${limit} OFFSET ${offset}`
        console.log(query)
        db.query(query, (err, res) => {
            if(err){
